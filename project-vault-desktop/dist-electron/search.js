...
function isArtifactIndex(absPath) {
    if (!absPath.toLowerCase().endsWith(`${path.sep}index.html`))
        return false;
...
export async function fullSystemSearchWithProgress(onProgress) {
    // 루트 후보
    let roots;
    ...
    for (const r of roots)
        total += await walkCount(path.resolve(r));
    if (total === 0)
        total = 1; // 0분모 방지
    let scanned = 0;
    const results = [];
    async function walkScan(dir) {
        try {
            const entries = await fs.readdir(dir, { withFileTypes: true });
            for (const ent of entries) {
                const full = path.join(dir, ent.name);
